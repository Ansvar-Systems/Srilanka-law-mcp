#!/usr/bin/env tsx
/**
 * Sri Lankan Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://lawnet.gov.lk';
const PORTAL_NAME = 'Sri Lanka Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/srilanka-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Sri Lankan Law MCP — Data Freshness Check');

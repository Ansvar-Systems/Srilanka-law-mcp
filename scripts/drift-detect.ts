#!/usr/bin/env tsx
/**
 * Drift detection for Sri Lanka Law MCP.
 *
 * Checks if upstream Sri Lanka Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Sri Lanka Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'srilanka-law-mcp/1.0 drift-detect' },
      });
